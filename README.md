...
In this task we have to write a function that measures the balance factor of a binary tree. In which *tree* is a pointer to the root node of the binary tree.
It returns 0 if tree is NULL.

## Sixteenth task: Is full
#### Prototype `int binary_tree_is_full(const binary_tree_t *tree);`
In this task we have to write a function that checks if a binary tree is full. In which *tree* is a pointer to the root node of a binary tree.
It returns 0 if tree is NULL.

## Seventeenth task: Is perfect
#### Prototype `int binary_tree_is_perfect(const binary_tree_t *tree);`
In this task we have to write a function that checks if a binary tree is perfect. In which *tree* is a pointer to the root node of a binary tree.
It returns 0 if tree is NULL.

## Eighteenth task: Sibling
#### Prototype `binary_tree_t *binary_tree_sibling(binary_tree_t *node);`
In this task we have to write a function that finds the sibling of a node. In which *node* is a pointer to the node to find the sibling.
The function returns a pointer to the sibling node, it returns NULL if *node* or the *parent* is NULL or if *node* has no sibling.

## Nineteenth task: Uncle
#### Prototype `binary_tree_t *binary_tree_uncle(binary_tree_t *node);`
In this task we have to write a function that finds the uncle of a node. In which *node* is a pointer to the node to find the uncle.
The function returns a pointer to the sibling node, it returns NULL if *node* of the *parent* is NULL or if *node* has no sibling.
